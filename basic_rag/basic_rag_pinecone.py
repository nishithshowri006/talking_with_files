...
def add_to_vectorstore(
    file_path, client: openai.OpenAI, index: Index, chunk_size=CHUNK_SIZE
):

    # get chunks of the text
    ...
    # we want to add these to the database
    to_upsert = zip(ids, [embeddings], meta)

    # upsert the data into vectorstore
    index.upsert(vectors=list(to_upsert), namespace=file_path)


def chat_completions(client: openai.OpenAI, context: str, current_input: str):
...
"""
    ...
    # add user query, system prompt, we can later develop chat history also so that model is context aware later
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": f"input prompt : {current_input}"},
    ]
...
def main():
    ...
    pc = Pinecone()
    # to create a new index isntead of exisgting db
    # index = pc.create_index(
    #     name="Sample Index",
    #     dimension=1536,  # this is for the our embeddin model,
    #     spec=ServerlessSpec(cloud="aws", region="us-east-1"),
    #     metric="cosine",  # we need to use cosine similarity
    # )
    # use an existing db
    index = pc.Index(host=os.getenv("PINECONE_HOSTNAME"))

    # get the text chunks, embeddings
    add_to_vectorstore(file_paths, client, index, chunk_size=512)
    ...
    try:
        while True:
            ...
            context = " ".join(
                [entry["metadata"]["text"] for entry in result["matches"]]
            )
            # get the repsosne generated by the model
            response = chat_completions(client, context, current_input)
            print(response)

    except KeyboardInterrupt:
        index.delete(delete_all=True, namespace=file_paths)
        print("Session Ended")
